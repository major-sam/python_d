...
def check_trace_points(points, target):
    result = []
    prev_dist = None
    for point in points:
        current_dist = my_utils.get_distance_between(point, target)
...
class ArcTracer(Drone):
    ...
    def build_arc(self, target, left_position):
        ...
        shifted_circle = my_utils.shift_circle(trace_circle,
                                               ct.get_nearest_point_index(self.coord, trace_circle))
