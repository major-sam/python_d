...
class Headquarters:
    ...
    def __init__(self):
        ...
        self.get_roles(1, Drone.scene)
        self.victims = []

    def new_soldier(self, soldier):
        number_drones = len(self.soldiers) + 1
        self.get_roles(number_drones, soldier.have_gun)
    ...
    def get_roles(self, number_drones, have_gun):
        if have_gun:
            transports = 0  # number_drones // 3
            collectors = 0  # transports
            combats = number_drones // 2  # number_drones // 2 #- transports - collectors
            guard = number_drones - combats
...
class DevastatorDrone(Drone):
    ...
    def born_soldier(self):
        ...
        if self.have_gun:
            self.attack_range = self.gun.shot_distance - 50
    ...
    def move_to_step(self, object):
        ...
        new_coord = Point(x=int(self.coord.x + vec.x), y=int(self.coord.y + vec.y))
    ...
    def on_born(self):
        ...
        if self.have_gun:
            ...
            if point_attack:
                self.actions.append(['turn', point_attack, 1])
